# The R Menagerie {#thermenagerie}

```{r r-menagerie, echo = FALSE, out.width = '100%', fig.link = 'https://en.wikipedia.org/wiki/Menagerie'}
...
Fig. \@ref(fig:r-objects) might seem a little overwhelming at first, but don't let it intimidate you. Its meaning will become clear as we proceed. First, however, let's try to figure out how to make one of these objects in R.
...
Basically, what is happening here is that the statement creates an object (`5.137`) in the R environment, specifically the _global_ environment, and then assigns that object to the name `bob`. This is why it is sometimes referred to as an _assignment_ statement [@rcoreteam2020introduction]. It is not just creating an object, but assigning it to a name.

Why assign a name? Well, if we just typed
...
Now, you may think, "I could just create `5.137` any time I wanted, let it die, and just create it again. It's not _that_ hard to type five characters, after all." In response to this flirtation with anarchy, two arguments are recommended. First, it is certainly true that one number is not hard to type, but it is rare that you will need just one. Suppose you had an object consisting of ten thousand numbers, would you care to type _that_ each time you needed it? Second, when you make an object with a name, R has your back. If you type `6.137` instead of `5.137`, for example, R will not protest, but if you type `boo` instead of `bob`, R will say, "nope."
...
In sum, we create objects in R and give them names, so that it is easier to call them up and work with them.
```
...
As an honorable mention, we will also discuss _factors_ here. These are not strictly their own data type. They are actually just integers (mostly for historical reasons), though they are typically represented as character strings when printed to the console. That said, only Socrates would be so fastidious as to insist on the distinction, regardless of whatever damage that might do to communicating their intent. So, let's not be like Socrates. Let's treat factors as an honorary data type in their own right, on a level with the others (even if they aren't, technically).
...
_What is it for?_ Categorizing things! For example, "Bob is a _Quebecois_" or "Xie is a _Homo sapien_."
...
Vectors come in two general flavors, atomic and complex, which differ in one crucial respect. While atomic vectors are limited to one data type, complex vectors can contain many data types. As shown in Fig. \@ref(fig:r-objects), atomic vectors include scalars, vectors, and matrices. Complex vectors include lists and data.frames. 
...
A matrix-vector is an atomic vector with _dimensions_, meaning the vector is "folded," so to speak, into rows and columns. As you can see, a matrix has the shape of a data table, like what you would find in an Excel spreadsheet, which is sometimes referred to as "rectangular" data.
