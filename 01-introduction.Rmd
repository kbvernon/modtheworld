...
This book provides an advanced introduction to doing statistics using the R programming language and environment [@rcoreteam2020language], equipping the reader with the programming skills necessary to do statistics cheaply, efficiently - and perhaps most importantly - in a reproducible fashion.

## Who is this book for?

It is primarily geared toward students who have some proficiency with basic statistical concepts. In the United States, that would be advanced undergraduates and beginning graduate students. While it does cover much of the same ground as an introductory course, it covers those topics in greater detail and with an eye toward expressing and exploring those statistical concepts within the R programming language and environment. The hope is that being a better R programmer will make you a better statistician and that being a better statistician will make you a better R programmer.    
...
So, why write another one? The best way to answer that question is probably with the big idea motivating the book, specifically the intuition that _all_ of statistics may be thought of as a framework for not only building models of the world, but evaluating them. This idea guides the book's layout and content, providing a fresh approach to statistics and statistical programming.
...
i. __Concisde__ code sticks to the main purpose of the analysis and does not include extraneous operations.  
