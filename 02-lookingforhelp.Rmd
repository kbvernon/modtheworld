# Looking for Help {#lookingforhelp}
...
As you make your first hesitant forays into the R programming environment - and even when you have become sufficiently familiar with the place to know your way around, you will on occasion experience the jarring dislocation of an R error, a typically dark red eruption of your R console, perhaps symbolic of your code exploding before your eyes. Here is one of the more infamous errors you are likely to encounter:

<p style='color:red'>object of type 'closure' is not subsettable</p>  

Nevermind what this particular error means.^[Though check out Jenny Bryan's talk at the 2020 RStudio Conference: https://rstudio.com/resources/rstudioconf-2020/object-of-type-closure-is-not-subsettable/] The point is that it can be terribly frustrating when you encounter it or something like it. Worse still, you will sometimes have so much trouble finding a solution, you start to think maybe all this programming and statistics we are using to learn about nature and the world isn't worth it. This difficulty, at least early on in your R programming career, will often boil down to the fact that you haven't learned enough R yet to even know how to ask the right question. 

I like to refer to this unhappy circumstance as the _Google Search Paradox_. It goes something like this: 

> You have a problem with your code, so you go online to find a solution. However, you do not know the right technical words to describe your problem, so your Google searching gets you no where. And if you did have the right words, you probably wouldn't need to do the Google searching anyway! So, it turns out, getting an answer with Google's help is either unnecessary or impossible!

Of course, the situation is not nearly so dire as this. In truth, you always know at least a little about the problem and can thus Google your way through to an answer - eventually, anyway. Life is fleeting, however, and time is short, so you should probably avoid the brute force approach, relying instead on searching _efficiently_. To help you with that (and to help you get _better_ with that), this chapter provides a brief annotated list of where to look for answers, starting from within R itself!  
...
1. From within R, you can use the `help()` and `?...` functions to access short-form documentation. Examples include `help("plot")`, `?plot`. This will call up the function's documentation page and display it in your computer's graphical device. All R functions have a similar page, which provides a brief description of the function, the parameters that it takes and how these change the way it works. It is also worth scrolling down - at the bottom of the help page will be some examples of usage which you can run to get a feel for how the function works. 
...
[RStudio](https://rstudio.com/) is an open-source software development company that provides tools for working with a number of programming languages; though, as its name suggests, it is primarily geared toward R. Far and away the most useful tool RStudio offers is Rstudio Desktop, which is part of RStudio's Integrated Development Environment (or, IDE)). If you wanted to boil it down to a simple description, it's just a user-friendly interface to the R environment - though big emphasis on the "user-friendly" (for more information, see [Chapter 9. R and RStudio](#randrstudio)).  
...
Others have likely asked the same question you want to ask, so you not always need to post your question. If you do, however, you need to make sure you provide R users with all the information and resources they need to help troubleshoot your code. This involves providing a "reproducible example" (typically shortened to _reprex_). There are two essential ingredients to a reprex: 
...
A lot has been written about how to put together a reprex, so rather than belabor the point here, it is perhaps best to direct you to the [reprex package](https://reprex.tidyverse.org/), which will walk you through the process of submitting a help request on the various forums mentioned above.  
