...
R follows some simple rules for filtering rows and subsetting variables that you should be aware of. These concern whether we subsetting single or multiple rows and single or multiple columns, so we can represent these rules as a 2x2 contingency matrix, with the value in each cell indicating the resulting vector type:
...
2. Specifying _multiple rows BUT only a single column_ will return a vector (a variable).
