...
```{block2, type = 'rmdnote'}
List objects provide another really useful tool for workspace management, though their utility might not be as obvious to R users early in their career. A very common use case, however, involves data objects that have different values but share identical structures (like data.frames with the same number of columns, all having the same names). This will happen, for example, when you are doing bootstrap resampling or simulations and want to generate a new model for each set of data. There are more sophisticated ways to manage such data, but a list is a good option too. Rather than having, say, ten thousand unique data.frames in your R environment, you have a single list to hold them all.
```
...
```{r 14-directory-structure, echo = FALSE, out.width = "30%"}
